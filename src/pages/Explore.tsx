  ...
  return (
    <div className="min-h-screen px-6 md:px-16 py-10">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[#7a7a90] mt-1" role="status" aria-live="polite">
      ...
      <div className="flex flex-wrap gap-3 mb-3">
        ...
        <div className="relative flex-1 min-w-[220px]">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#4a4a60] text-sm pointer-events-none">
            🔍
          </span>
          <input
            type="search"
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7a7a90] hover:text-white text-xs cursor-pointer p-1"
        ...
        {/* Type chips */}
        <div className="flex gap-2" role="group" aria-label="Filter by skill type">
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <button
            onClick={clearAll}
            className="px-5 py-2.5 rounded-xl border border-white/15 text-white text-sm font-medium hover:bg-white/[0.05] transition-colors cursor-pointer"
          >
            Clear Filters
          </button>
        </motion.div>
